...
def gen_eval_set(profile_features,labels,modlist,available_sample,n_ex):
  eval_idx = random.sample([i for i in range(len(labels))],n_ex)
  return np.array([profile_features[idx] for idx in eval_idx]),np.array([labels[idx] for idx in eval_idx]),np.array([modlist[idx] for idx in eval_idx])
...
def run_trial(profile_features,labels,modlist,this_train_sizes,results,n):
  ...
  for i in range(1,len(this_train_sizes)):
    ...
    eval_set_size = int(0.1*len(labels))
    eval_features,eval_labels,eval_modlist = gen_eval_set(profile_features,labels,modlist,available_sample,eval_set_size)
...
json.dump(results.tolist(),open("datalimit10_boostrapped_predmae_10sim.json","w"))
json.dump(this_train_sizes.tolist(),open("trainsize_datalimit10_bootstrapped_predmae_10sim.json","w"))
