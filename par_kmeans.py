...
num_procs = 100
num_trials = 1 #for kmeans always 1
this_train_sizes = np.linspace(0.05,1,20)
results = Manager().list([0 for i in range(len(this_train_sizes)*num_trials*num_procs)])
...
def run_trial(profile_features,labels,modlist,this_train_sizes,results,n):
  ...
  for i in range(len(this_train_sizes)):
    cur_size = min(int(np.ceil(len(labels)*this_train_sizes[i])),len(labels))
    kmeans = KMeans(n_clusters=cur_size, random_state=n).fit(profile_features)
    closest, _ = pairwise_distances_argmin_min(kmeans.cluster_centers_, profile_features)
    best_X_train,best_y_train,best_modlist_train,closest = gen_eval_set(profile_features,labels,modlist,closest)
...
results = np.array(results).reshape((num_trials*num_procs,len(this_train_sizes)))
results = np.sum(results,axis=0)
results /=(num_trials * num_procs)

json.dump(results.tolist(),open("kmeans_100sim.json","w"))
json.dump(this_train_sizes.tolist(),open("trainsize_kmeans_100sim.json","w"))
