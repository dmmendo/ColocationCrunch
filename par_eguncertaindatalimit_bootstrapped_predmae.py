...
def gen_eval_set(profile_features,labels,modlist,eval_set,cur_reg,indivRuntimes,available_mask,n_ex):
  epsilon = 0.7
  if np.random.binomial(1,epsilon) == 1:
    probs = uncertainty(cur_reg,profile_features,modlist,indivRuntimes,available_mask)
    if np.sum(probs) > 0:
      eval_idx = np.random.choice([i for i in range(len(labels))],n_ex,p=probs/np.sum(probs))
      for idx in eval_idx:
        eval_set.add(idx)
  return np.array([profile_features[idx] for idx in eval_set]),np.array([labels[idx] for idx in eval_set]),np.array([modlist[idx] for idx in eval_set]),eval_set
...
def run_trial(profile_features,labels,modlist,this_train_sizes,results,n):
  ...
  for i in range(1,len(this_train_sizes)):
    ...
    eval_sample_size = 1
    eval_features,eval_labels,eval_modlist,eval_set = gen_eval_set(profile_features,labels,modlist,eval_set,cur_reg,indivRuntimes,available_mask,eval_sample_size)
...
json.dump(results.tolist(),open("eguncertaindatalimit_boostrapped_predmae_10sim.json","w"))
json.dump(this_train_sizes.tolist(),open("trainsize_eguncertaindatalimit_bootstrapped_predmae_10sim.json","w"))
