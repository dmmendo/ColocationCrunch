...
def gen_eval_set(profile_features,labels,modlist,eval_set,available_set,cur_ensemble):
  this_eval_set = random.sample(list(eval_set),int(0.1*len(labels)))
  this_perturb_set = available_set - eval_set
  if len(this_perturb_set) > 0:
    this_perturb_set = random.sample(list(this_perturb_set),min(len(this_perturb_set),int(0.1*len(labels))))
    eval_features = np.concatenate((np.array([profile_features[idx] for idx in this_eval_set]),np.array([profile_features[idx] for idx in this_perturb_set])))
    eval_labels = np.concatenate((np.array([labels[idx] for idx in this_eval_set]),cur_ensemble.predict([profile_features[idx] for idx in this_perturb_set])))
    eval_modlist = np.concatenate((np.array([modlist[idx] for idx in this_eval_set]),np.array([modlist[idx] for idx in this_perturb_set])))
  else:
    eval_features = np.array([profile_features[idx] for idx in this_eval_set])
    eval_labels = np.array([labels[idx] for idx in this_eval_set])
    eval_modlist = np.array([modlist[idx] for idx in this_eval_set])
...
def run_trial(profile_features,labels,modlist,this_train_sizes,results,n):
  ...
  eval_set = set(random.sample([i for i in range(len(labels))],int(0.3*len(labels))))
  for i in range(1,len(this_train_sizes)):
    ...
    eval_features,eval_labels,eval_modlist,eval_set = gen_eval_set(profile_features,labels,modlist,eval_set,available_sample,cur_reg)
...
json.dump(results.tolist(),open("perturbdatalimit_boostrapped_predmae_10sim.json","w"))
json.dump(this_train_sizes.tolist(),open("trainsize_perturbdatalimit_bootstrapped_predmae_10sim.json","w"))
