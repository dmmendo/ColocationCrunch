# -*- coding: utf-8 -*-
"""ColocateDataCollection_template.ipynb
...
for n in range(num_trials):
  ...
  for i in range(1,len(this_train_sizes)):
    ...
    uncertain_vals.sort(key=lambda x: x[1])
    for idx in range(num_to_profile):
      available_mask[uncertain_vals[idx][0]] = 0
    new_train_feat = np.array([profile_features[uncertain_vals[idx][0]] for idx in range(num_to_profile)])
    new_train_label = np.array([labels[uncertain_vals[idx][0]] for idx in range(num_to_profile)])
    cur_X_train = np.concatenate((cur_X_train,new_train_feat))
    cur_y_train = np.concatenate((cur_y_train,new_train_label))
    reg = RandomForestRegressor().fit(cur_X_train,cur_y_train)
    results[i] += MAE(labels,reg.predict(profile_features),modlist,indivRuntimes)
results /=num_trials

json.dump(results.tolist(),open("min_uncertainty_100sim.json","w"))
json.dump(this_train_sizes.tolist(),open("trainsize_min_uncertainty_100sim.json","w"))