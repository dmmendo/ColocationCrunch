...
def fillspace_eval_set(num_trials,size,profile_features,labels,modlist):
  fill_val_max = float('-inf')
  best_set = None
  for j in range(num_trials):
    cur_set = set(random.sample([i for i in range(len(labels))],int(size*len(labels))))
    cur_features,cur_labels,cur_modlist,_ = gen_eval_set(profile_features,labels,modlist,cur_set)
    tmp_val = fillspace_func(cur_features)
    if tmp_val > fill_val_max:
      fill_val_max = tmp_val
      best_set = cur_set
  return best_set
...
def run_trial(profile_features,labels,modlist,this_train_sizes,results,val_results,n):
  ...
  eval_set = fillspace_eval_set(1000000,0.1,profile_features,labels,modlist)
...
json.dump(avg_results.tolist(),open("avg_fillspacedatalimit10_boostrapped_predmae_100sim.json","w"))
json.dump(min_results.tolist(),open("min_fillspacedatalimit10_boostrapped_predmae_100sim.json","w"))
json.dump(max_results.tolist(),open("max_fillspacedatalimit10_boostrapped_predmae_100sim.json","w"))

json.dump(avg_val_results.tolist(),open("avg_val_fillspacedatalimit10_boostrapped_predmae_100sim.json","w"))
json.dump(min_val_results.tolist(),open("min_val_fillspacedatalimit10_boostrapped_predmae_100sim.json","w"))
json.dump(max_val_results.tolist(),open("max_val_fillspacedatalimit10_boostrapped_predmae_100sim.json","w"))


json.dump(this_train_sizes.tolist(),open("trainsize_fillspacedatalimit10_bootstrapped_predmae_100sim.json","w"))
