...
def run_trial(profile_features,labels,modlist,this_train_sizes,results,n):
  ...
  for i in range(1,len(this_train_sizes)):
    ...
    for j in range(len(available_sample)):
      ...
      #sample_costs.append(np.sum(uncertainty(reg,profile_features,modlist,indivRuntimes,available_mask)))
      sample_costs.append(np.sum(uncertainty(reg,profile_features,modlist,indivRuntimes,np.ones(len(labels)))))
...
json.dump(results.tolist(),open("boostrapped_predtotaluncertain_10sim.json","w"))
json.dump(this_train_sizes.tolist(),open("trainsize_bootstrapped_predtotaluncertain_10sim.json","w"))
